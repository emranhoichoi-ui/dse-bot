...
def analyze(stocks):
    ...
    for s in stocks:
        ...
        if ltp>0 and abs(ltp-f618)/ltp<0.02:score+=2;tags.append("Fib 0.618 ✨")
        elif ltp>0 and abs(ltp-f382)/ltp<0.02:score+=1;tags.append("Fib 0.382")
...
def find_ew(stocks):
    ...
    for s in stocks:
        ...
        bp=(s['ltp']-s['low'])/rng
        n618=s['ltp']>0 and abs(s['ltp']-f618)/s['ltp']<0.025
        n382=s['ltp']>0 and abs(s['ltp']-f382)/s['ltp']<0.025
...
def ai_summary(buys,ew_stocks,dsex):
    ...
    lines="".join([f"{s['symbol']}:৳{s['ltp']}({s['change']:+.1f}%)\n" for s in buys[:5]])
...
async def send_signals(bot):
    ...
    try:
        stocks=fetch_stocks()
        if not stocks:
            await bot.send_message(chat_id=CHAT_ID,text="❌ ডেটা পাওয়া যায়নি।")
            return
        dsex=get_dsex()
...
async def cmd_start(u:Update,ctx:ContextTypes.DEFAULT_TYPE):
    await u.message.reply_text(
        ...
        "/signal — এখনই সিগনাল দেখুন\n"
        "/ew — Elliott Wave 2/4 স্টক\n"
        "/top — আজকের Top Gainers\n\n"
        "🕕 প্রতিদিন সন্ধ্যা ৬টায় সিগনাল আসবে।",
        parse_mode='Markdown')
...
async def cmd_top(u:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    stocks=fetch_stocks()
    if not stocks:
        await u.message.reply_text("ডেটা পাওয়া যায়নি।")
        return
...
async def post_init(app):
    ...
    sched.start()
    log.info("✅ Scheduler চালু — প্রতিদিন ৬:০০ PM")
